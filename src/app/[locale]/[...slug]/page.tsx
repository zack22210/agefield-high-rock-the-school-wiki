...
export async function generateMetadata({ params }: { params: Promise<{ locale: Locale; slug: string[] }> }): Promise<Metadata> {
  ...
  if (slug.length === 1 && CONTENT_TYPES.includes(slug[0])) {
    ...
    const title = ctMessages?.overviewTitle || `${ctTitle} — Agefield High Wiki`;
    const description = ctMessages?.overviewDescription || `Browse all ${ctTitle.toLowerCase()} guides for Agefield High: Rock the School.`;
    const pathname = `/${ct}`;
    return { title, description, alternates: { canonical: locale === "en" ? pathname : `/${locale}${pathname}`, languages: languageAlternates(pathname) }, openGraph: { title, description, url: localizedUrl(pathname, locale), siteName: "Agefield High: Rock the School Wiki", images: [`${siteUrl}/images/hero.webp`] }, twitter: { card: "summary_large_image", title, description, images: [`${siteUrl}/images/hero.webp`] } };
  }
...
async function NavigationPage({ locale, contentType }: { locale: Locale; contentType: string }) {
  ...
  const listData = { "@context": "https://schema.org", "@type": "ItemList", name: `${contentType} — Agefield High: Rock the School Wiki`, itemListElement: items.map((item, index) => ({ "@type": "ListItem", position: index + 1, url: localizedUrl(`/${contentType}/${item.slug}`, locale), name: item.metadata.title })) };
